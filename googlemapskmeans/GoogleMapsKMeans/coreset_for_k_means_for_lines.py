...
class CorsetForKMeansForLines:
    ...
    def alpha_beta_approximation(self, L, k, sample_size):
        ...
        while size > minimum_number_of_lines and size > sample_size:
            ...
            Q = Q.get_farthest_lines_to_centers(G, int(size*3/8), "by number")
            size = Q.get_size()
        if size > 0:
    ...
    def coreset(self, L, k, m):
        ...
        if cost_to_B == 0:
            ...
            sensitivities = sensitivity_first_argument_normalized.reshape(-1) + sensitivity_second_argument_normalized.reshape(-1) #according to line 5 in Alg. 1 in paper
